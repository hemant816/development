...
def index(request, extra_context=None, user=AnonymousUser()):
    ...
    context['clectures'] = CourseOverview.objects.all().filter(pk__in=cid, start__gte=today, catalog_visibility="both").order_by('start')
    ...
    context['index_upcoming_courses'] = CourseOverview.objects.all().filter(pk__in=index_cid, start__gte=today).order_by('start')[::-1]
    # query to display all courses
    context['index_ongoing_courses'] = CourseOverview.objects.all().filter(pk__in=index_cid,start__lte=today).order_by('start')[::-1]
